...
class Symbol(object):
    ...
    def __str__(self, indent = 0):
        ...
        return " "*indent + "{}({})".format(self.__class__.__name__, x)
...
class L (Terminal): # non-empty line: '<" "*indent><text>'
    text = ""
    def __str__(self, indent = 0):
        return "{}: {}".format(super(L, self).__str__(indent), self.text)
...
class DH (L):  # description: a line of type ' <description>::<inner_text>'
    ...
    description = None
    def __str__(self, indent = 0):
        return "{}: '{}' :: '{}'".format(super(L, self).__str__(indent),self.description, self.inner_text)
...
class Parser(object):
    ...
    def parse_BL(self, indent):
        ...
        if i == 0: pass # print "Error", sys.exit(1)
